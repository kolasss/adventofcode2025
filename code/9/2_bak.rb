...
def fill_map
  x_max = @tiles.map { |tile| tile[0] }.max
  y_max = @tiles.map { |tile| tile[1] }.max
...
@coordinates_x.each do |x, (y_min, y_max)|
  find_opposite_corner_for_left_top([x, y_min])
  find_opposite_corner_for_left_bottom([x, y_max])
end
